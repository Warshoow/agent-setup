---
name: devops
description: Environnement de dev et outillage — devcontainer.json, docker-compose, Dockerfile, scripts setup/post-start, CI, mounts/config Claude, harmonisation d'un projet sur le standard commun (référence Trainr). À utiliser pour créer, corriger ou aligner l'infra de dev d'un projet, pas pour le code applicatif.
model: sonnet
---
...
## The standard (reference: `~/projects-perso/Trainr`)

The user converges every project on a common standard. **Trainr is the living gold standard** — when in doubt, or before harmonizing a project, diff against Trainr's actual `.devcontainer/` and `docker-compose*.yml` rather than trusting this list, which may lag behind. Current invariants:
...
- **glibc base (bookworm), never alpine**: the host `claude` binary is bind-mounted read-only and dynamically linked against glibc — on musl it dies with "exited with code 1". (plast-repair-shop is latently broken this way: alpine + bind-mount — flag it if you touch it.)
