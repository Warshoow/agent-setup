---
name: evolve
description: Boucle instinct → skill — promouvoir les patterns récurrents de la mémoire persistante en skills réutilisables. Relit les mémoires de tous les projets (configs pro et perso), détecte les feedbacks/procédures revenus plusieurs fois, propose une promotion en skill dans ~/skills/ (validation explicite avant création). Trigger : /evolve, « promeus mes mémoires », « qu'est-ce qui mériterait de devenir une skill ? ».
---
...
3. **Proposition.** Présenter les candidats via AskUserQuestion (multiSelect) :
   nom kebab-case, description projetée (avec triggers), une ligne de
   justification citant les mémoires sources. **Ne rien créer sans sélection
   explicite.** Zéro candidat solide = le dire et s'arrêter là.

4. **Promotion.** Pour chaque skill retenue :
...
- Une skill = une procédure. Pas de fourre-tout.
